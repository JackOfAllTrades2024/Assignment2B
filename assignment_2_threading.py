...
# Local date and time updated once per second
# In a thread to speed up processing of overall system
def update_datetime():
...
# Create a new thread that will run the update_datetime function
datetime_thread = threading.Thread(target=update_datetime)

# Start the new thread
datetime_thread.start()

# Function to update the information section based on the selected source
...
# Function to download the webpage and extract the time and title of the latest news article based on the selected radio button, threading on the download function 
# to stop the GUI from freezing, fixing the bottleneck
def confirm_button_click():
    ...
    if selected_info_source == '9news':
        ...
        def download_and_process_webpage():
        ...
        download_thread = threading.Thread(target=download_and_process_webpage)
        # Start the new thread
    ...
    # THE AGE NEWS
        ...
        # Create a new thread that will run the download_and_process_webpage function
        download_thread = threading.Thread(target=download_and_process_webpage)
...
# Function to open the selected source n default browser
...
# Function to save the reliability rating to the database
